...
// is reachable. The registry backend drives all image operations through the
// `skopeo` CLI, which is therefore a hard requirement: the registry is selected
// only when skopeo is installed.
...
export async function detectCapabilities() {
  // A reachable remote registry is the preferred backend. All of its image
  // operations run through skopeo, so skopeo must be present for it to be used.
  const registry = registryConfigFromEnv();
  if (registry && (await probe(registry)) && (await hasBinary('skopeo'))) {
...
export async function initialize() {
  ...
  // Warn when REGISTRY_URL was configured but we did not select the registry
  // backend, distinguishing "skopeo missing" from "unreachable".
  if (caps.engine !== 'registry') {
    const reg = registryConfigFromEnv();
    if (reg) {
      const reason = (await hasBinary('skopeo'))
        ? 'the registry was not reachable'
        : "'skopeo' was not found (it is required for the registry backend)";
      console.warn(
        `Note: REGISTRY_URL is set but ${reason}. ` +
